...
## Unreleased
- **Bug fix: de-duplication scan timed out (HTTP 500 from the reverse proxy after ~120 s) on large calendars** — the rewritten scan resolved each synced event's featured image by loading an `EM_Event` object per event (`em_get_event()`), which on a calendar with many repeating occurrences meant thousands of object loads and blew past the front-end proxy's ~120 s timeout (well under PHP's own limits, so raising memory/time didn't help). The featured images are now resolved in a **single SQL join** (mapping → the Events Manager events table → `postmeta._thumbnail_id`), with an EM-API fallback; the flyer cleanup got the same bulk `event_id → post_id` lookup. The scan now completes in seconds

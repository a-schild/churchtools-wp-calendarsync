...
- **Bug fix: a sync killed while importing an image left a half-finished copy behind, and the next sync uploaded yet another copy** — WordPress can need 30 s or more to generate all image sizes of a large photo, and hosts with a hard request limit killed the sync in the middle of it. The imported image is now registered (ChurchTools id, featured image) *before* its sizes are generated, and flagged until they are complete; the next sync finishes the missing sizes (WordPress keeps the ones already made) instead of importing the image again. The log now shows how long the size generation took and the peak memory

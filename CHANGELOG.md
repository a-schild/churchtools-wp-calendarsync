...
## 2026-09-19
- Release v1.5.4
- **Security: flyer import could write files outside its temp folder (path traversal)** — flyers were saved through the ChurchTools library's `downloadToPath()`, which writes the file under the name exactly as ChurchTools returns it; a flyer file name containing `../` could therefore place a file anywhere the web server may write. The flyer is now fetched directly and written under a sanitised name into a private, randomly named temp folder that is removed afterwards (temp files were previously never cleaned up). This also fixes flyers whose name contains spaces or umlauts, which never imported because the written and the read-back file names differed
- **Security: the saved API token is only ever sent to the saved ChurchTools URL** — the "Validate Connection", "Load Calendars" and "Load Resource Types" buttons and saving the settings re-used the hidden saved token for whatever URL was entered, so it could be sent to another server. Changing the URL now requires entering the token again
- **Security: ChurchTools URLs must resolve to a public address** — the connection buttons and saving a new URL now reject loopback, private, link-local and reserved addresses (e.g. `localhost`, `192.168.x.x`, the `169.254.169.254` cloud metadata service), so the plugin cannot be used to probe internal services. An unchanged saved URL keeps working. For a self-hosted ChurchTools server in the local network, add `define('CTWPSYNC_ALLOW_PRIVATE_URLS', true);` to wp-config.php (or use the `ctwpsync_allow_private_urls` filter)

## 2026-09-19
- Release v1.5.3
- **Bug fix: a replaced image in ChurchTools did not show up in WordPress, the event kept the old picture** — when the new image had the same filename as the old one and the event was in the same month, the sync matched it to the existing media library file by name and re-used the **old** attachment, while recording the new ChurchTools image id as already synced — so later syncs never looked at it again. A filename match is now only re-used if the file content is byte-identical; otherwise the new image is imported. Events already affected are repaired automatically on the next sync: when an event's featured image is not the attachment imported for its current ChurchTools image, the image is checked again (downloaded once, and re-used without a new upload if it turns out to be identical). Note that the ChurchTools image now also wins over a featured image set manually in WordPress for synced events that have a ChurchTools image
- **Bug fix: a failed image download was recorded as synced** — the new ChurchTools image id was stored in the mapping even when the image could not be imported (for example while the old download link required authentication, fixed in 1.5.2), so the image was never retried. It is now only recorded once the image is actually attached
- **Improvement: one media file can now stand for several ChurchTools images** — the same picture uploaded to several ChurchTools appointments has a different ChurchTools id each time. The "Image de-duplication" clean-up merges those identical files into one, but kept only one of the ids, so the others were no longer recognised and could be imported again. An attachment now keeps all of its ChurchTools image ids (the clean-up carries over the ids of the copies it removes), and the sync re-uses an identical existing file for a new id instead of uploading a copy — library-wide: every imported image now carries a content fingerprint (`_ctwpsync_image_md5`, recorded on import and filled in for existing images as they are synced), so the same picture is found even under another name or in another month folder (always verified against the actual file before re-use). If an earlier test build left `-1` copies behind, run the clean-up once to merge them
- **New: the image clean-up also merges identical images with a different name or in another month folder** — e.g. the same picture used by two ChurchTools appointments and imported as `2026/10/pic-1.jpg` and `2026/11/pic-1.jpg`, which the existing clean-up (same name, same folder only) could not see. This second pass is deliberately strict: it only considers images this plugin imported (never your own uploads), requires the original files to be byte-identical (SHA-256), and deletes a copy only when nothing refers to it except featured images (which are re-pointed to the kept copy) — no post content or excerpt, post meta or option may mention its file path (also JSON-escaped, as page builders store it) or its attachment id. Anything found, even by coincidence, keeps the copy and is listed in the result notes. The scan shows these sets as "Identical, other name/folder"
- **Safety: the image clean-up no longer treats two media entries that point at the same physical file as duplicates** — deleting one of them would have deleted the file the other still uses
- **Bug fix: a sync killed while importing an image left a half-finished copy behind, and the next sync uploaded yet another copy** — WordPress can need 30 s or more to generate all image sizes of a large photo, and hosts with a hard request limit killed the sync in the middle of it. Because each event is saved in a database transaction, the kill also rolled back the new media entry, leaving only its files behind. The image sizes are now generated *after* the event's transaction is committed, with the image flagged until they are complete; the next sync finishes the missing sizes (WordPress keeps the ones already made) instead of importing the image again. The log now shows how long the size generation took and the peak memory

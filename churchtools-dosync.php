...
function processCalendarEntry(
	...
	global $wpdb;
	// Attachment whose image sizes still need generating. Done only after the COMMIT
	// below: it can take 30 s for a large photo, and a host kill inside the transaction
	// would roll back the imported attachment and leave its files orphaned on disk.
	$pendingImageId = null;
	...
	//                }
		...
		$wpdb->query('COMMIT');
		if ($pendingImageId) {
			ctwpsync_finish_image_sizes((int) $pendingImageId);
		}
	} 
	catch (Exception $e)
	{
...
function downloadEventImage(string $fileURL, string $fileName, int $postID, \DateTime $eventDate, string $ctBaseUrl = '', ?int $ctImageId = null, ?int $currentAttachmentId = null): ?int {
    ...
    if ( ! $upload_file['error'] ) {
      ...
      if ( ! is_wp_error( $attachment_id ) ) {
         ...
         // The sizes themselves are generated by processCalendarEntry() after its
         // transaction is committed (ctwpsync_finish_image_sizes): that can take 30 s
         // for a large photo, and if the host kills the sync meanwhile, the next run
         // finds this attachment and finishes the missing sizes instead of uploading
         // yet another "name-1.jpg" copy.
         if ( $ctImageId !== null ) {

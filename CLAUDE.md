...
|---------|------------|------------|
| 1.5.4   | 2026-09-19 | Security: (1) flyer path traversal — library `File::downloadToPath()` wrote the unsanitised CT file name; replaced by `downloadFlyerToMediaLibrary()` (sanitised name, random private temp dir, cleaned up; also fixes flyers with spaces/umlauts in the name); (2) the saved API token is only reused for the saved URL (`ctwpsync_urls_match()`) in the connection/calendar/resource-type AJAX endpoints and on settings save; (3) SSRF: `ctwpsync_is_valid_remote_url()` requires a host resolving only to public IPs, opt-out `CTWPSYNC_ALLOW_PRIVATE_URLS` / filter `ctwpsync_allow_private_urls` |
| 1.5.3   | 2026-09-19 | Bug fix (images): replaced CT images never reached WP — a failed import (pre-1.5.2 auth download) or a same-filename match to the old attachment still recorded the new `ct_image_id`; now recorded only after a successful import, filename matches must be byte-identical, and events whose featured image isn't stamped with the current CT id are re-checked (self-heal). `_ctwpsync_ct_image_id` is multi-valued (`ctwpsync_get_ct_image_ids`/`ctwpsync_add_ct_image_id`); library-wide reuse by content fingerprint `_ctwpsync_image_md5`. Image sizes generated **after** the per-event transaction commits (`ctwpsync_finish_image_sizes`, `_ctwpsync_sizes_pending`, resumes via `wp_update_image_subsizes`) — host kills (~2 min limit on the reporting site) previously rolled back the attachment and caused re-uploads. Image clean-up: strict cross-name/folder pass (`ctwpsync_dedupe_identical_images`, reference check `ctwpsync_find_attachment_references`), shared-physical-file guard. Flyers: failed import no longer records the new `ct_flyer_id`. Status tab "Reset sync lock" (`ctwpsync_reset_sync_lock`). Sync Now starts an already-queued/overdue event instead of "Failed to schedule sync". `monolog` 3.12.0; tested up to WP 7.1.1 |
| 1.5.2   | 2026-09-07 | Bug fix (images): ChurchTools now requires authentication on the old `fileUrl` download link, so appointment images stopped importing and the `em_image_attr` direct-embed option served an unloadable `<img src>`. `processCalendarEntry()` now uses the image model's `getImageUrl()` (the unauthenticated REST download URL) with `?crop=original` appended for the uncropped original (PR #25, @JonFStr). Filename/extension validation, the `str_starts_with($fileURL, $ctBaseUrl)` SSRF check and image de-duplication are unaffected — they key on `getName()`/`getId()`, not the URL; flyers keep the authenticated `downloadToPath()` path. Also: the image debug log line now logs `$imageURL` so it shows the URL actually fetched (it previously omitted `?crop=original`); in-constraint dependency bumps (`guzzle` 7.15.2→7.15.5, `psr7` 2.13.0→2.13.1, `promises` 2.5.1→2.5.3, `monolog` 3.10.0→3.11.0), `composer audit` clean, majors still blocked by the fork |
| 1.5.1   | 2026-07-30 | Security (dependencies): `guzzlehttp/guzzle` 7.14.1→7.15.2 + `guzzlehttp/psr7` 2.12.5→2.13.0, clearing 4 moderate Dependabot advisories (redirect `Referer` fragment leak, unbounded response cookies, host-only cookie scope, `Proxy-Authorization` sent to origin). Major bumps (guzzle 8, psr7 3, promises 3, doctrine/cache 2) are blocked by the `5pm-hdh/churchtools-api` fork's `guzzlehttp/guzzle: ^7` / `doctrine/cache: ^1.11` constraints |
| 1.5.0   | 2026-07-15 | Media de-duplication + admin UX release. **Prevention:** the sync no longer re-downloads/re-uploads the same ChurchTools image or flyer for every event/repeating occurrence (which produced `name-1.jpg`…/`flyer-1.pdf`… copies) — attachments are stamped with `_ctwpsync_ct_image_id`/`_ctwpsync_ct_flyer_id` and reused library-wide; `wp_image_id` is now persisted. **Cleanup tools** (Tools tab): "Image de-duplication" and optional "Flyers" Scan/Clean up that collapse pre-existing duplicates — detection is filename-base + byte-identical (`md5`) hash, catching orphaned copies the mapping doesn't reference; the scan uses a single bulk query (no per-image `LIKE`, no hashing) and the cleanup runs in time-boxed auto-continuing batches so large libraries don't hit the reverse-proxy timeout. **Admin UX:** settings page reorganised into five tabs (Connection/Sync Options/Status/Tools/Logs). **Observability:** dedupe results logged to the plugin log at INFO; hardened AJAX (output buffering so PHP notices can't corrupt the JSON, memory raise, fatal-logging shutdown handler). |
...
**Image de-duplication:** attachments imported for event images are stamped with a `_ctwpsync_ct_image_id` post-meta (the CT file id). Before downloading, `downloadEventImage()` looks that id up library-wide via `get_attachment_id_by_ct_image_id()` and reuses the single existing attachment (shared across all events/repeating occurrences using the same CT image, regardless of upload month) instead of letting `wp_upload_bits()` create `name-1.jpg`/`name-2.jpg` copies. `wp_image_id` is persisted in the mapping on insert/update; if the image import fails, the new `ct_image_id` is **not** recorded, so the next sync retries. The stamp is **multi-valued** (one `_ctwpsync_ct_image_id` row per CT id): the same picture uploaded to several CT appointments has several CT ids that all resolve to one attachment. Read/write it only via `ctwpsync_get_ct_image_ids()` / `ctwpsync_add_ct_image_id()` (main plugin file), never `update_post_meta` (which would collapse the ids). If no stamped attachment exists, the image is downloaded and a candidate — a sync-imported attachment with the same content fingerprint `_ctwpsync_image_md5` (library-wide; set on import, backfilled lazily via `ctwpsync_image_md5()`), the event's current featured image, or a same-name file in the month folder — is reused (and gets the id added) **only if byte-identical** (`md5`); a plain filename match used to resolve a replaced CT image to the old attachment (fixed in 1.5.3). New uploads are stamped + set as thumbnail and flagged `_ctwpsync_sizes_pending`; the sizes are generated by `ctwpsync_finish_image_sizes()` **after** `processCalendarEntry()`'s per-event `START TRANSACTION`/`COMMIT` (never inside it: a host kill would roll back the attachment and orphan its files). It generates/resumes sizes (`wp_update_image_subsizes`) and clears the flag, so a kill mid-generation is resumed instead of re-uploaded. On updates where the mapping's `ct_image_id` is unchanged, `processCalendarEntry()` still re-checks the image when the event's featured image isn't stamped with that CT id (self-heals events hit by that bug). Pre-existing duplicates (imported before this meta existed) are cleaned up on demand via the settings-page "Image de-duplication" Scan/Clean up buttons → `ctwpsync_dedupe_images()` (AJAX action `ctwpsync_dedupe_images`, in the main plugin file). Detection is **filename-base + hash**, not mapping-only: it starts from plugin image attachments (stamped, in `wp_image_id`, or a synced event's current featured image), finds siblings sharing the same base filename (strips WP `-N`/`-scaled` suffixes) in the same folder, confirms byte-identical via `md5_file`, keeps the lowest attachment id, re-points featured images + `wp_image_id`, stamps the kept one with the **union** of all members' CT image ids, and deletes the rest (skips any whose URL is in post content). This catches **orphaned** duplicates (the usual leftovers of the old re-download bug); an earlier mapping-only version missed them and reported 0. Featured images of synced events are resolved with a single SQL join (mapping → `{prefix}em_events` → `postmeta._thumbnail_id`), **not** `em_get_event()` per event (which timed out the scan on large calendars). After that same-name pass finishes, a stricter second pass `ctwpsync_dedupe_identical_images()` merges byte-identical (SHA-256) **stamped** attachments across names/folders; it deletes a copy only if `ctwpsync_find_attachment_references()` finds no reference by file path (plain + JSON-escaped) or attachment id in posts, postmeta (except `_thumbnail_id`, which is re-pointed) and options. Both passes skip attachments sharing one physical file. The clean-up is **time-boxed** (~45 s/request, `$budgetSeconds`): it returns `more => true` when it stops early and the dashboard JS auto-loops the AJAX call until done, so a proxy timeout can't kill a large clean-up mid-way.

**Flyer de-duplication:** flyers (event files with "flyer" in the name) use the same scheme with a **separate** meta key `_ctwpsync_ct_flyer_id` and `get_attachment_id_by_ct_flyer_id()` — event-file ids and appointment-image ids are different id spaces. `downloadEventImage` handles images (featured image); the flyer branch in `processCalendarEntry()` handles flyers (linked in `post_content` via `addFlyerLink()`, not set as thumbnail). `wp_flyer_id` was already persisted in the mapping; a failed flyer import does not record the new `ct_flyer_id`, and a mapped flyer stamped with a different CT id is re-imported. Pre-existing flyer duplicates are collapsed by a **separate** optional cleanup, `ctwpsync_dedupe_flyers()` (AJAX action `ctwpsync_dedupe_flyers`, "Flyers (optional)" buttons under the Image de-duplication section): per CT flyer it keeps the lowest still-existing attachment, stamps the meta, **rewrites the flyer URL in each affected event's `post_content`** to the canonical URL, sets `wp_flyer_id`, and deletes redundant attachments only when nothing (post content or featured-image) still references them. It is kept separate from the image tool because it edits published event content.
...
4. Sync status tracked via transients: `churchtools_wpcalendarsync_in_progress` (10-min lease with heartbeat; admins can release it early via the Status tab "Reset sync lock" button → AJAX `ctwpsync_reset_sync_lock`), `churchtools_wpcalendarsync_lastupdated`, `churchtools_wpcalendarsync_lastsyncduration`
...
- **Flyer download:** never use the library's `File::downloadToPath()` — it writes `<path>/<name>` with the unsanitised CT file name (path traversal). Use `downloadFlyerToMediaLibrary()` (`churchtools-dosync.php`): `requestFileContent()` → sanitised name in a random temp dir → `uploadFromLocalFile()` → temp cleaned up.
- **Saved API token:** the AJAX helpers (`validate_connection`, `get_calendars`, `get_resource_types`) and `save_ctwpsync_settings()` only reuse the saved token when the URL matches the saved one (`ctwpsync_urls_match()`).
- **SSRF:** `ctwpsync_is_valid_remote_url()` requires http(s) and a host resolving only to public IPs (`ctwpsync_resolve_host()`); opt-out for self-hosted LAN servers via `CTWPSYNC_ALLOW_PRIVATE_URLS` / filter `ctwpsync_allow_private_urls`. Applied to the AJAX endpoints and to saving a **changed** URL (an unchanged saved URL is not re-checked).

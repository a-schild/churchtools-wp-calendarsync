...
- **New: "Image de-duplication" cleanup tool on the settings page** — the prevention fix above only stops *new* duplicates; images imported by older versions have no `_ctwpsync_ct_image_id` meta. The settings page now has a **Scan for duplicates** button (dry run — reports how many CT images have duplicates and how many attachments would be removed, changes nothing) and a **Clean up duplicates** button (with a confirm) that, per CT image, keeps the lowest/original attachment id as a single shared copy, stamps its `_ctwpsync_ct_image_id` meta, re-points every event's featured image and the mapping's `wp_image_id` to it, and deletes the redundant attachments (files + thumbnails). A duplicate still used as another post's featured image is left in place and reported. Backed by the nonce- + `manage_options`-protected `ctwpsync_dedupe_images` AJAX action (`ctwpsync_dedupe_images()`). Fully orphaned leftover files (nobody's featured image) are not deleted — use the Media Library "Unattached" filter for those
